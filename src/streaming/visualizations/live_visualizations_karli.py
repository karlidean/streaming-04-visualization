...
def update_live_chart(
    *,
    ...
    # Create a new y value from the running tax collected metric.
    new_y = float(message["total_tax_collected"])
    ...
    axis.set_title("Cumulative Tax Collected by Message")
    axis.set_xlabel("Message Number")
    axis.set_ylabel("Total Tax Collected ($)")
